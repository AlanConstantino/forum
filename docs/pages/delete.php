...
<html>
  ...
  <body>
    <div class="all-posts">
      <h3>Click on a post to delete it.</h3>
  	  <?php
        ...
        if (strpos($full_url, 'fail'))
          echo '<p id="post-fail">Failed to delete post.</p>';

        if (strpos($full_url, 'success'))
          echo '<p id="post-success">Succesfully deleted post.</p>';
        ...
        if(mysqli_num_rows($valid_query) > 0){
          while($row = mysqli_fetch_assoc($valid_query)){
              ...
              echo '<a href="../authenticate/authenticate-delete.php?post_id=' . $row['id'] . '">';
